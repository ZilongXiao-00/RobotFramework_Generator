...
// ─── 模块级别的步骤代码生成（组件外，方便 sync 时调用）──────
function genStepsCode(nodes: any[], level = 1): string {
  let c = '';
  const ind = '    '.repeat(level);
  nodes.forEach((s: any, i: number) => {
    if (s.isComment) {
      c += `${ind}# ${s.args?.text || ''}\n`;
    } else if (s.isCustomCode) {
      (s.customCode || '').split('\n').forEach((l: string) => { c += `${ind}${l}\n`; });
    } else if (s.isContainer) {
      let line = `${ind}${s.keyword}`;
      Object.values(s.args || {}).forEach((v: any) => { if (v) line += `    ${v}`; });
      (s.extraArgs || []).forEach((v: any) => { if (v) line += `    ${v}`; });
      c += line + '\n';
      c += s.children?.length > 0 ? genStepsCode(s.children, level + 1) : `${ind}    # TODO\n`;
      const ku = s.keyword.toUpperCase();
      if (['FOR', 'WHILE', 'IF', 'ELSE IF', 'ELSE'].includes(ku)) {
        const next = nodes[i + 1];
        if (!(['IF', 'ELSE IF', 'ELSE'].includes(ku) && next && ['ELSE IF', 'ELSE'].includes(next.keyword?.toUpperCase()))) {
          c += `${ind}END\n`;
        }
      }
    } else {
      let line = ind;
      const vv = (s.outputVars || []).filter((v: string) => v.trim());
      if (vv.length > 0) line += vv.join('    ') + '    ';
      if (s.modifier) line += `${s.modifier}    `;
      line += s.keyword;
      Object.values(s.args || {}).forEach((v: any) => { if (v) line += `    ${v}`; });
      (s.extraArgs || []).forEach((v: any) => { if (v) line += `    ${v}`; });
      c += line + '\n';
...
/** 给定一个 userKeyword 对象，生成完整的 .robot 关键字文本块 */
function buildKeywordBlock(kw: any): string {
  let code = `${kw.name || '新关键字'}\n`;
  if (kw.desc) code += `    [Documentation]    ${kw.desc}\n`;
  const va = (kw.args || []).filter((a: string) => a.trim());
  if (va.length > 0) {
    code += `    [Arguments]    ${va.map((a: string) => a.startsWith('${') ? a : `\${${a}}`).join('    ')}\n`;
  }
  if (kw.steps?.length > 0) {
    code += genStepsCode(kw.steps);
  }
  const vr = (kw.returnVars || []).filter((r: string) => r.trim());
  if (vr.length > 0) {
    code += `    [Return]    ${vr.map((r: string) => r.startsWith('${') ? r : `\${${r}}`).join('    ')}\n`;
  }
  return code;
}
...
const INITIAL_KEYWORD_LIBRARY = [
  ...
  { category: 'Control Flow', name: 'ELSE IF', args: ['condition'], desc: 'ELSE IF', isContainer: true },
  { category: 'Control Flow', name: 'ELSE', args: [], desc: 'ELSE', isContainer: true },
...
function AuthPage({ onLogin }: { onLogin: (t: string, u: any) => void }) {
  ...
  const [name, setName] = useState(''); const [showPw, setShowPw] = useState(false);
  const [loading, setLoading] = useState(false); const [error, setError] = useState('');
  const submit = async () => {
    setError(''); if (!email || !password) { setError('邮箱和密码不能为空'); return; }
    setLoading(true);
    try {
      const body: any = { email, password }; if (mode === 'register') body.name = name || email;
      const data = await apiFetch(`/auth/${mode}`, { method: 'POST', body: JSON.stringify(body) });
      localStorage.setItem('rf_token', data.token); localStorage.setItem('rf_user', JSON.stringify(data.user));
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#1e1e1e] border border-gray-800 rounded-2xl p-8">
          <div className="flex gap-1 mb-8 bg-[#141414] p-1 rounded-lg">
            {(['login','register'] as const).map(m=><button key={m} onClick={()=>{setMode(m);setError('');}} className={`flex-1 py-2 text-sm font-bold rounded-md ${mode===m?'bg-[#F27D26] text-white':'text-gray-500 hover:text-gray-300'}`}>{m==='login'?'登录':'注册'}</button>)}
          </div>
          <div className="space-y-4">
            ...
            <div><label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">密码</label>
              <div className="relative"><input type={showPw?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()} placeholder={mode==='register'?'至少6位':'密码'} className="w-full px-4 py-3 bg-[#141414] border border-gray-700 rounded-lg text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#F27D26] pr-10"/>
                <button onClick={()=>setShowPw(!showPw)} className="absolute right-3 top-3.5 text-gray-600 hover:text-gray-400">{showPw?<EyeOff size={16}/>:<Eye size={16}/>}</button></div>
            </div>
            {error&&<div className="px-4 py-3 bg-red-900/30 border border-red-800/50 rounded-lg text-red-400 text-xs">{error}</div>}
            <button onClick={submit} disabled={loading} className="w-full py-3 bg-[#F27D26] hover:bg-[#d96b1f] disabled:opacity-50 text-white font-bold text-sm rounded-lg mt-2">{loading?'处理中...':mode==='login'?'登录':'创建账号'}</button>
          </div>
        </div>
      </div>
    </div>
...
function MyCasesPanel({ token, onLoad, onClose }: { token: string; onLoad: (c:any,id:number,n:string)=>void; onClose:()=>void }) {
  const [cases, setCases] = useState<any[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  useEffect(()=>{apiFetch('/cases',{},token).then(d=>setCases(d.cases)).catch(e=>setError(e.message)).finally(()=>setLoading(false));}, []);
  const load = async (id:number)=>{try{const d=await apiFetch(`/cases/${id}`,{},token);onLoad(d.case.content,d.case.id,d.case.name);onClose();}catch(e:any){setError(e.message);}};
  const del = async (e:React.MouseEvent,id:number)=>{e.stopPropagation();if(!confirm('确认删除？'))return;try{await apiFetch(`/cases/${id}`,{method:'DELETE'},token);setCases(p=>p.filter(c=>c.id!==id));}catch(e:any){setError(e.message);}};
  return (
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading?<div className="text-center py-12 text-gray-400 text-sm">加载中...</div>:error?<div className="text-center py-12 text-red-500 text-sm">{error}</div>:cases.length===0?<div className="text-center py-12"><FileText size={40} className="mx-auto text-gray-200 mb-3"/><p className="text-gray-400 text-sm">还没有保存的用例</p></div>:(
            <div className="space-y-2">{cases.map(c=>(
              <div key={c.id} onClick={()=>load(c.id)} className="group flex items-center gap-4 p-4 border border-gray-100 rounded-xl hover:border-[#F27D26] hover:bg-orange-50/30 cursor-pointer transition-all">
                ...
                <button onClick={e=>del(e,c.id)} className="opacity-0 group-hover:opacity-100 p-1.5 hover:bg-red-50 rounded-lg"><Trash2 size={14} className="text-red-400"/></button>
              </div>
            ))}</div>
          )}
        </div>
...
function SaveModal({ defaultName, currentCaseId, onSave, onClose }: { defaultName:string; currentCaseId:number|null; onSave:(n:string,o:boolean)=>void; onClose:()=>void }) {
  const [name, setName] = useState(defaultName); const [overwrite, setOverwrite] = useState(!!currentCaseId);
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e=>e.stopPropagation()}>
        <h3 className="font-bold text-gray-900 mb-1">保存到云端</h3><p className="text-xs text-gray-400 mb-5">用例将保存到你的账号下</p>
...
// ─── Sync Keyword Modal ───────────────────────────────────
// 选目标库、确认关键字信息，同步完整步骤实现到 .robot 文件
function SyncKeywordModal({
  keyword, libraries, onSync, onClose
}: {
  keyword: any; // 完整的 userKeyword 对象，含 steps/returnVars
  ...
  // 预览生成的代码
  const previewCode = buildKeywordBlock({ ...keyword, name: kwName, args: kwArgs, desc: kwDoc });
  ...
  const handleSync = async () => {
    ...
    setSyncing(true);
    try {
      await onSync(selectedLibId, { ...keyword, name: kwName.trim(), args: kwArgs.filter((a: string) => a.trim()), desc: kwDoc });
      onClose();
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          ...
          <div><h3 className="font-bold text-gray-900 text-sm">同步到关键字库</h3><p className="text-[10px] text-gray-400">将完整实现写入云端 .robot 文件</p></div>
        ...
        <div className="flex-1 overflow-y-auto">
          {libraries.length === 0 ? (
            <div className="py-12 text-center text-gray-400 text-sm">还没有导入任何库文件，请先上传 .robot/.resource</div>
          ) : (
            <div className="flex gap-0 h-full">
              ...
              <div className="flex-1 p-5 space-y-4 border-r border-gray-100">
                <div>
                  ...
                  <div className="space-y-1">
                    {libraries.map(lib=>(
                      <label key={lib.id} className={`flex items-center gap-3 p-3 border rounded-lg cursor-pointer ${selectedLibId===lib.id?'border-[#F27D26] bg-orange-50':'border-gray-200 hover:border-gray-300'}`}>
                        <input type="radio" name="lib" checked={selectedLibId===lib.id} onChange={()=>setSelectedLibId(lib.id)} className="accent-[#F27D26]"/>
                        <span className="text-sm font-mono text-gray-700">{lib.file_name}</span>
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">说明</label>
              ...
              <div className="w-80 flex flex-col">
                <div className="px-4 py-2 bg-gray-800 text-gray-400 text-[10px] font-bold uppercase tracking-wider flex items-center gap-2">
                  <Code size={12}/> 将写入 .robot 的代码
                </div>
                <pre className="flex-1 bg-[#1e1e1e] text-[#d4d4d4] p-4 font-mono text-xs overflow-auto whitespace-pre">{previewCode}</pre>
              </div>
            </div>
          )}
        </div>

        {error && <div className="mx-5 mb-3 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-red-600 text-xs">{error}</div>}
...
export default function App() {
  ...
  const rebuildSidebar = (files: LibraryFile[]) => {
    ...
    setLibrary([...INITIAL_KEYWORD_LIBRARY,...cloudKws]);
  };
  ...
  const handleLibFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLibraryLoading(true);
    setLogs(p=>[...p,`[INFO] 上传 ${files.length} 个文件解析中...`]);
    try {
      ...
      data.results?.forEach((r:any)=>setLogs(p=>[...p,`[CLOUD] ${r.file_name}: ${r.keyword_count} 个关键字已入库`]));
      data.errors?.forEach((er:any)=>setLogs(p=>[...p,`[ERROR] ${er.file}: ${er.error}`]));
    } catch(e:any){setLogs(p=>[...p,`[ERROR] 上传失败: ${e.message}`]);}
    finally{setLibraryLoading(false);e.target.value='';}
  };

  // ── 删除单个关键字（同步到云端 .robot 文件）───────────────
  const handleDeleteKeywordFromLibrary = async (kwName: string, libId: number) => {
    const lib = userLibraryFiles.find(f=>f.id===libId);
    if (!lib) return;
    if (!confirm(`从 "${lib.file_name}" 中删除关键字 "${kwName}"？\n这将同时修改云端的 .robot 文件内容。`)) return;
    try {
      const data = await apiFetch(`/libraries/${libId}/delete-keyword`, {
        method: 'PUT',
        body: JSON.stringify({ name: kwName })
      }, token);
      // 更新本地 keywords 列表和 file_content
      setUserLibraryFiles(prev=>prev.map(f=>f.id===libId?{...f,keywords:data.keywords,file_content:data.file_content}:f));
      setLibrary(prev=>prev.filter(k=>!(k.name===kwName&&(k as any)._libId===libId)));
      setLogs(p=>[...p,`[CLOUD] 已从 ${lib.file_name} 删除关键字: ${kwName}`]);
    } catch(e:any){setLogs(p=>[...p,`[ERROR] 删除失败: ${e.message}`]);}
  };

  // ── 同步自定义关键字到云端库（含完整步骤实现）────────────
  const handleSyncKeywordToLib = async (libId: number, kwEntry: any) => {
    const lib = userLibraryFiles.find(f=>f.id===libId);
    if (!lib) throw new Error('库不存在');

    // 生成完整的关键字代码块（含步骤）
    const kwCode = buildKeywordBlock(kwEntry);

    const meta = { name: kwEntry.name, args: (kwEntry.args||[]).filter((a:string)=>a.trim()), doc: kwEntry.desc||'', tags: [] };

    const data = await apiFetch(`/libraries/${libId}/sync-keyword`, {
      method: 'PUT',
      body: JSON.stringify({ name: kwEntry.name, keyword_code: kwCode, meta })
    }, token);
    ...
    setLibrary(prev=>{
      ...
      const newEntry = {category:lib.file_name,name:kwEntry.name,args:meta.args,desc:meta.doc,isContainer:false,isCustomCode:false,isComment:false,_libId:libId};
      if(idx>=0) return prev.map((k,i)=>i===idx?newEntry:k);
      return [...prev,newEntry];
    });
    setLogs(p=>[...p,`[CLOUD] 关键字 "${kwEntry.name}" 已同步到 ${lib.file_name}`]);
  };

  // ── 删除整个库文件 ────────────────────────────────────────
  const handleDeleteLibraryFile = async (lib: LibraryFile) => {
    if (!confirm(`删除库 "${lib.file_name}"？\n云端文件将同时删除。`)) return;
    try {
      await apiFetch(`/libraries/${lib.id}`,{method:'DELETE'},token);
      setUserLibraryFiles(p=>p.filter(f=>f.id!==lib.id));
      setLibrary(p=>p.filter(k=>k.category!==lib.file_name));
      setLogs(p=>[...p,`[INFO] 已删除库: ${lib.file_name}`]);
    } catch(e:any){setLogs(p=>[...p,`[ERROR] 删除失败: ${e.message}`]);}
  };
  ...
  const handleSave = async (name:string,overwrite:boolean)=>{
    ...
    try{
      ...
      if(overwrite&&currentCaseId){await apiFetch(`/cases/${currentCaseId}`,{method:'PUT',body:JSON.stringify({name,content})},token);setCurrentCaseName(name);}
      else{const data=await apiFetch('/cases',{method:'POST',body:JSON.stringify({name,content})},token);setCurrentCaseId(data.case.id);setCurrentCaseName(name);}
      setSaveStatus('saved');setLogs(p=>[...p,`[CLOUD] 用例已保存: ${name}`]);setTimeout(()=>setSaveStatus('idle'),2500);
    }catch(e:any){setSaveStatus('error');setLogs(p=>[...p,`[ERROR] 保存失败: ${e.message}`]);setTimeout(()=>setSaveStatus('idle'),3000);}
  };
  const handleLoadCase = (content:any,caseId:number,caseName:string)=>{
    if(content.testCases){setTestCases(content.testCases);setActiveTestCaseId(content.testCases[0]?.id||'tc_1');}
    ...
    setLogs(p=>[...p,`[CLOUD] 已加载: ${caseName}`]);
  };
  ...
  const setSteps = (upd:any)=>{
    if(activeTab==='testcases'){setTestCases(p=>p.map(tc=>tc.id!==activeTestCaseId?tc:{...tc,steps:typeof upd==='function'?upd(tc.steps):upd}));}
    else if(activeUserKeywordId){setUserKeywords(p=>p.map(kw=>kw.id!==activeUserKeywordId?kw:{...kw,steps:typeof upd==='function'?upd(kw.steps):upd}));}
  };
  const updateATC = (u:any)=>setTestCases(p=>p.map(tc=>tc.id===activeTestCaseId?{...tc,...u}:tc));
  const updateAUK = (u:any)=>setUserKeywords(p=>p.map(kw=>kw.id===activeUserKeywordId?{...kw,...u}:kw));
  ...
  const confirmImport = ()=>{
    ...
    reader.onload=(ev)=>{
      try{
        ...
        for(const line of rawLines){const t=line.trim();if(t.startsWith('...')){if(lines.length>0)lines[lines.length-1]+='    '+t.slice(3).trim();}else lines.push(line.replace(/\r$/,''));}
        let section:string|null=null,parsedVars:any[]=[],parsedTCs:any[]=[],curTC:any=null,stack:any[]=[],parsedSettings:string[]=[],parsedKws:string[]=[];
        ...
        for(const line of lines){
          ...
          if(section==='settings'){if(t)parsedSettings.push(line);}
            ...
            if(!line.startsWith(' ')&&!line.startsWith('\t')){finalize();curTC={id:`tc_${Date.now()}_${Math.random().toString(36).substr(2,9)}`,name:t,teardown:'',tags:'',steps:[]};stack=[];}
              ...
              else if(t.startsWith('#')){const s={id:`step_${Date.now()}_${Math.random().toString(36).substr(2,9)}`,keyword:'#',isComment:true,args:{text:t.slice(1).trim()}};(stack.length>0?stack[stack.length-1].children:curTC.steps).push(s);}
        ...
        setGlobalVars(parsedVars);setSettingsSection(parsedSettings.length>0?parsedSettings.join('\n'):'');
        setCustomKeywordsSection(parsedKws.length>0?parsedKws.join('\n'):'');
        setCurrentCaseId(null);setCurrentCaseName('');
        setLogs(p=>[...p,`[INFO] 导入完成: ${pendingImportFile!.name}`]);
      }catch(e:any){setLogs(p=>[...p,`[ERROR] 解析失败: ${e.message}`]);}
  ...
  // ── Tree ops ──────────────────────────────────────────────
  const findStep=(ns:any[],id:string):any=>{for(const n of ns){if(n.id===id)return n;if(n.children){const f=findStep(n.children,id);if(f)return f;}}return null;};
  const updateNode=(ns:any[],id:string,u:any):any[]=>ns.map(n=>n.id===id?{...n,...u}:n.children?{...n,children:updateNode(n.children,id,u)}:n);
  ...
  const handleStepDragOver=(e:React.DragEvent,id:string,isCont:boolean)=>{e.preventDefault();e.stopPropagation();if(draggedStepId===id)return;const r=(e.currentTarget as HTMLElement).getBoundingClientRect();const y=e.clientY-r.top;setDropTarget({id,position:y<r.height*.25?'before':(isCont&&y<r.height*.75)?'inside':'after'});};
  ...
  const generateCode=():string=>{
    ...
    c+='*** Test Cases ***\n';testCases.forEach(tc=>{c+=`${tc.name||'Test Case'}\n`;if(tc.tags)c+=`    [Tags]    ${tc.tags}\n`;c+=genStepsCode(tc.steps);if(tc.teardown)c+=`    [Teardown]    ${tc.teardown}\n`;c+='\n';});
    if(userKeywords.length>0){c+='*** Keywords ***\n';userKeywords.forEach(kw=>{c+=buildKeywordBlock(kw)+'\n';});}
  ...
  const renderSteps=(nodes:any[],parentId:string|null=null):React.ReactNode=>{
    ...
    return nodes.map(step=>{
      ...
      if(step.isComment)return(
        <React.Fragment key={step.id}>{isDT&&dropTarget!.position==='before'&&<div className="h-1 bg-[#F27D26] rounded my-1"/>}
          <div draggable onDragStart={e=>handleStepDragStart(e,step.id)} onDragOver={e=>handleStepDragOver(e,step.id,false)} onDrop={e=>handleStepDrop(e,step.id)} onDragEnd={handleDragEnd} onClick={e=>{e.stopPropagation();setSelectedStepId(step.id);}} className={`group relative flex items-center bg-green-50/50 border rounded-lg shadow-sm cursor-pointer mb-2 px-3 py-2 ${selectedStepId===step.id?'border-green-500 ring-1 ring-green-500':'border-green-200 hover:border-green-300'} ${draggedStepId===step.id?'opacity-50':''}`}>
            <GripVertical size={16} className="text-green-400 mr-2 cursor-grab"/><Hash size={14} className="text-green-600 mr-1"/>
            <span className="text-green-700 font-mono text-sm flex-1">{step.args.text||'Comment'}</span>
            <button onClick={e=>handleDeleteStep(e,step.id)} className="text-green-400 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={16}/></button>
          </div>{isDT&&dropTarget!.position==='after'&&<div className="h-1 bg-[#F27D26] rounded my-1"/>}
        </React.Fragment>
      );
      return(
        <React.Fragment key={step.id}>{isDT&&dropTarget!.position==='before'&&<div className="h-1 bg-[#F27D26] rounded my-1"/>}
          <div draggable onDragStart={e=>handleStepDragStart(e,step.id)} onDragOver={e=>handleStepDragOver(e,step.id,step.isContainer)} onDrop={e=>handleStepDrop(e,step.id)} onDragEnd={handleDragEnd} onClick={e=>{e.stopPropagation();setSelectedStepId(step.id);}} className={`group relative flex flex-col bg-white border rounded-lg shadow-sm cursor-pointer mb-2 ${selectedStepId===step.id?'border-[#F27D26] ring-1 ring-[#F27D26]':'border-gray-300 hover:border-gray-400'} ${draggedStepId===step.id?'opacity-50':''} ${isDT&&dropTarget!.position==='inside'?'ring-2 ring-[#F27D26]':''}`}>
            <div className="flex items-stretch">
              ...
              <div className="flex-1 p-3 flex flex-col justify-center"><div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex gap-2 ml-2 flex-wrap">
                  {Object.entries(step.args||{}).map(([k,v]:[string,any])=>v&&<span key={k} className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded border border-gray-200">{k}: <span className="font-mono">{v}</span></span>)}
                  {(step.extraArgs||[]).map((v:any,i:number)=>v&&<span key={`e${i}`} className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded border border-gray-200">arg{i+1}: <span className="font-mono">{v}</span></span>)}
                </div>
              </div></div>
              <button onClick={e=>handleDeleteStep(e,step.id)} className="w-10 flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-tr-lg opacity-0 group-hover:opacity-100"><Trash2 size={16}/></button>
            </div>
            {step.isContainer&&(
    ...
    <div className="flex flex-col h-screen bg-[#E4E3E0] text-[#141414] font-sans overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 py-3 bg-[#141414] text-[#E4E3E0] shadow-md z-10 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={runSimulation} disabled={isRunning||steps.length===0} className="flex items-center gap-2 px-4 py-1.5 text-xs font-bold bg-gray-700 text-white rounded hover:bg-gray-600 disabled:opacity-50 border border-gray-600"><Play size={14} fill="currentColor"/> 运行</button>
          <div className="w-px h-6 bg-gray-700 mx-1"/>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 bg-white border-r border-gray-300 flex flex-col shadow-sm z-10">
          <div className="p-3 border-b border-gray-200 space-y-2">
            <div className="relative"><Search size={14} className="absolute left-2.5 top-2 text-gray-400"/><input type="text" placeholder="搜索关键字..." className="w-full pl-8 pr-3 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}/></div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded p-2 space-y-1">
                ...
                {userLibraryFiles.map(lib=>(
                  <div key={lib.id} className="flex items-center gap-1 group">
                    ...
                    <button onClick={()=>window.open(`${API_BASE}/libraries/${lib.id}/export?token=${token}`,'_blank')} title="下载 .robot" className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-400 hover:text-blue-500"><Download size={11}/></button>
            ...
            {categories.map(cat=>{
              ...
              return(
                <div key={cat} className="mb-2">
                  <div onClick={()=>toggleCategory(cat)} className="flex items-center gap-1 px-1 py-1.5 rounded hover:bg-gray-100 cursor-pointer group/cat select-none">
                    <span className="text-gray-400 flex-shrink-0">{isCollapsed?<ChevronRight size={13}/>:<ChevronDown size={13}/>}</span>
                    ...
                    <div className="space-y-1 mt-1 pl-1">
                      {catKws.map(kw=>(
                        <div key={kw.name} className="group/kw relative flex items-stretch">
                          <div draggable onDragStart={e=>handleDragStart(e,kw)} onDoubleClick={()=>insertNewStep(kw,null,'append')}
                            className={`flex-1 px-3 py-2 text-xs border cursor-grab active:cursor-grabbing hover:shadow-sm transition-all ${
                            ...
                            } ${(isCloud||(kw as any).isUserKeyword)?'rounded-l':'rounded'}`}
                          ...
                          {isCloud&&cloudLib&&(
                            <button onClick={()=>handleDeleteKeywordFromLibrary(kw.name,cloudLib.id)}
                              className="opacity-0 group-hover/kw:opacity-100 w-7 flex items-center justify-center bg-amber-50 border border-l-0 border-amber-200 rounded-r text-gray-300 hover:text-red-500 hover:bg-red-50 transition-colors"
                              title={`从 ${cat} 删除此关键字（同步到云端）`}>
                              <Trash2 size={11}/>
                            </button>
                          )}
                          {(kw as any).isUserKeyword&&(
                            <button onClick={()=>removeUserKeyword((kw as any)._kwId)}
                ...
                {activeTab==='testcases'?(
                  <>
                    <div className="flex items-end gap-1 mb-4 border-b border-gray-300 overflow-x-auto pt-2 px-2">
                      {testCases.map(tc=>(
                        <div key={tc.id} onClick={()=>setActiveTestCaseId(tc.id)} className={`group flex items-center gap-2 px-4 py-2 rounded-t-lg border-t border-l border-r cursor-pointer whitespace-nowrap ${activeTestCaseId===tc.id?'bg-white border-gray-300 text-[#F27D26] font-bold relative top-[1px]':'bg-gray-100 border-transparent text-gray-600 hover:bg-gray-200'}`}>
                        ...
                        <div className="p-4 space-y-4">
                          <div className="pb-4 border-b border-gray-100 space-y-4">
                            ...
                            <div><label className="block text-xs font-bold text-gray-700 mb-1">全局变量</label>
                              <div className="space-y-2">
                                {globalVars.map((v,i)=>(<div key={i} className="flex items-center gap-2"><input type="text" value={v.name} onChange={e=>{const n=[...globalVars];n[i].name=e.target.value;setGlobalVars(n);}} placeholder="${VAR}" className="w-1/3 px-2 py-1 text-xs font-mono border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]"/><span className="text-gray-400">=</span><input type="text" value={v.value} onChange={e=>{const n=[...globalVars];n[i].value=e.target.value;setGlobalVars(n);}} className="flex-1 px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]"/><button onClick={()=>setGlobalVars(globalVars.filter((_,idx)=>idx!==i))} className="p-1 text-gray-400 hover:text-red-500"><X size={14}/></button></div>))}
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              <div><label className="block text-xs font-bold text-gray-700 mb-1">用例名称</label><input type="text" value={activeTestCase.name} onChange={e=>updateATC({name:e.target.value})} className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]"/></div>
                              <div><label className="block text-xs font-bold text-gray-700 mb-1">标签</label><input type="text" value={activeTestCase.tags||''} onChange={e=>updateATC({tags:e.target.value})} className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]" placeholder="Done, Smoke"/></div>
                              <div><label className="block text-xs font-bold text-gray-700 mb-1">Teardown</label><input type="text" value={activeTestCase.teardown||''} onChange={e=>updateATC({teardown:e.target.value})} className="w-full px-2 py-1.5 text-xs font-mono border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]"/></div>
                            </div>
                          </div>
                        </div>
                      )}
                    ...
                    {activeUserKeyword?(
                      <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-6 overflow-hidden">
                        <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                          ...
                          {userLibraryFiles.length>0&&(
                            <button onClick={()=>setSyncModal({keyword:activeUserKeyword})}
                              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200 rounded-lg hover:bg-amber-100">
                              <BookOpen size={13}/> 同步到库（含步骤）
                            </button>
                          )}
                        </div>
                        <div className="p-4 space-y-4">
                          <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">名称</label><input type="text" value={activeUserKeyword.name} onChange={e=>updateAUK({name:e.target.value})} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#F27D26]"/></div>
                          <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">说明</label><textarea value={activeUserKeyword.desc} onChange={e=>updateAUK({desc:e.target.value})} rows={2} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#F27D26]"/></div>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">参数</label>
                              {activeUserKeyword.args.map((a:string,i:number)=>(<div key={i} className="flex gap-2 mb-2"><input type="text" value={a} onChange={e=>{const n=[...activeUserKeyword.args];n[i]=e.target.value;updateAUK({args:n});}} className="flex-1 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded text-xs" placeholder="${arg}"/><button onClick={()=>updateAUK({args:activeUserKeyword.args.filter((_:any,j:number)=>j!==i)})} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 size={14}/></button></div>))}
                              <button onClick={()=>updateAUK({args:[...activeUserKeyword.args,'']})} className="text-[10px] text-[#F27D26] font-bold hover:underline">+ 添加参数</button>
                            </div>
                            <div>
                              <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">返回值</label>
                              {activeUserKeyword.returnVars.map((r:string,i:number)=>(<div key={i} className="flex gap-2 mb-2"><input type="text" value={r} onChange={e=>{const n=[...activeUserKeyword.returnVars];n[i]=e.target.value;updateAUK({returnVars:n});}} className="flex-1 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded text-xs" placeholder="${result}"/><button onClick={()=>updateAUK({returnVars:activeUserKeyword.returnVars.filter((_:any,j:number)=>j!==i)})} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 size={14}/></button></div>))}
                              <button onClick={()=>updateAUK({returnVars:[...activeUserKeyword.returnVars,'']})} className="text-[10px] text-[#F27D26] font-bold hover:underline">+ 添加返回值</button>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white rounded-xl border border-dashed border-gray-300 p-12 text-center">
                        <FileText size={48} className="mx-auto text-gray-300 mb-4"/><p className="text-gray-500 mb-4">还没有自定义关键字</p>
                  ...
                  <div className="space-y-3">
                    ...
                    {steps.length===0?(
                      <div className="h-64 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center text-gray-400">
                        <Plus size={48} className="mb-4 opacity-50"/><p className="font-medium">从左侧拖拽关键字到此处</p><p className="text-xs mt-1">支持 IF/FOR 嵌套结构</p>
                      </div>
                    ):renderSteps(steps)}
                  </div>
                )}
                ...
                {selectedStep.isComment?(
                  ...
                  <>
                    ...
                    {!selectedStep.isContainer&&(
                      <div className="pt-4 border-t border-gray-100"><label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">执行修饰符</label>
                        <select value={selectedStep.modifier||''} onChange={e=>updateStep(selectedStep.id,{modifier:e.target.value})} className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:border-[#F27D26] bg-white">
                          <option value="">无</option><option value="Run Keyword And Continue On Failure">Run Keyword And Continue On Failure</option><option value="Run Keyword And Ignore Error">Run Keyword And Ignore Error</option><option value="Wait Until Keyword Succeeds">Wait Until Keyword Succeeds</option>
                        </select>
                      </div>
                    )}
                    <div className="pt-4 border-t border-gray-100"><label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">附加参数</label>
