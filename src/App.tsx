...
const INITIAL_KEYWORD_LIBRARY = [
  ...
  { category: 'Variables', name: 'Set Suite Variable', args: ['name', 'value'], desc: 'Makes a variable available within the suite.' },
  { category: 'Variables', name: 'Set Global Variable', args: ['name', 'value'], desc: 'Makes a variable available globally.' },
  ...
  { category: 'BuiltIn', name: 'Evaluate', args: ['expression'], desc: 'Evaluates the given expression in Python.' },
          ...
          <div className="flex gap-1 mb-8 bg-[#141414] p-1 rounded-lg">
            {(['login', 'register'] as const).map(m => (
              <button key={m} onClick={() => { setMode(m); setError(''); }}
                className={`flex-1 py-2 text-sm font-bold rounded-md transition-all ${mode === m ? 'bg-[#F27D26] text-white' : 'text-gray-500 hover:text-gray-300'}`}>
                {m === 'login' ? '登录' : '注册'}
              </button>
            ))}
          </div>
          <div className="space-y-4">
            {mode === 'register' && (
              <div>
                ...
                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="你的名字"
                  className="w-full px-4 py-3 bg-[#141414] border border-gray-700 rounded-lg text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#F27D26]" />
              </div>
            )}
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} placeholder={mode === 'register' ? '至少6位' : '请输入密码'}
                  className="w-full px-4 py-3 bg-[#141414] border border-gray-700 rounded-lg text-white text-sm placeholder-gray-600 focus:outline-none focus:border-[#F27D26] pr-10" />
                <button onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3.5 text-gray-600 hover:text-gray-400">
...
// ─── My Cases Panel ────────────────────────────────────────
function MyCasesPanel({ token, onLoad, onClose }: { token: string; onLoad: (content: any, id: number, name: string) => void; onClose: () => void; }) {
  ...
  const fmt = (d: string) => new Date(d).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-orange-50 rounded-lg flex items-center justify-center"><Cloud size={16} className="text-[#F27D26]" /></div>
            <div><p className="font-bold text-gray-900 text-sm">云端用例</p><p className="text-[10px] text-gray-400">{cases.length} 个已保存</p></div>
          </div>
                ...
                {cases.map(c => (
                  <div key={c.id} onClick={() => handleLoad(c.id)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400 font-mono mt-0.5">{fmt(c.updated_at)}</p>
                    </div>
                    <button onClick={e => handleDelete(e, c.id)} className="opacity-0 group-hover:opacity-100 p-1.5 hover:bg-red-50 rounded-lg">
...
// ─── Save Modal ────────────────────────────────────────────
function SaveModal({ defaultName, currentCaseId, onSave, onClose }: { defaultName: string; currentCaseId: number | null; onSave: (name: string, overwrite: boolean) => void; onClose: () => void; }) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
        ...
        {currentCaseId && (
          <label className="flex items-center gap-3 mb-5 cursor-pointer">
            <input type="checkbox" checked={overwrite} onChange={e => setOverwrite(e.target.checked)} className="w-4 h-4 accent-[#F27D26]" />
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => { if (name.trim()) { onSave(name.trim(), overwrite); onClose(); } }}
            className="flex-1 py-2.5 bg-[#F27D26] text-white text-sm font-bold rounded-lg hover:bg-[#d96b1f]">保存</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const loadUserLibraries = async () => {
    ...
    try {
      ...
      // 把云端库合并进 library state（去重）
      const cloudKeywords: any[] = [];
      files.forEach(file => {
      ...
      setLibrary(prev => {
        // 保留内置，移除旧的云端库，加入新的
        const builtIn = prev.filter(k => !userLibraryFiles.some(f => f.file_name === k.category));
        const existingNames = new Set(builtIn.map(k => k.name));
        const unique = cloudKeywords.filter(k => !existingNames.has(k.name));
        return [...builtIn, ...unique];
      });

      if (files.length > 0) {
        setLogs(prev => [...prev, `[CLOUD] 已自动加载 ${files.length} 个关键字库，共 ${cloudKeywords.length} 个关键字`]);
      }
    } catch (e: any) {
      console.error('[加载库失败]', e.message);
    } finally {
      setLibraryLoading(false);
    }
  };

  // ─── 上传 .robot/.resource 到后端解析 ───────────────────
  const handleLibFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLibraryLoading(true);
    setLogs(prev => [...prev, `[INFO] 正在上传并解析 ${files.length} 个文件...`]);

    try {
      const res = await fetch(`${API_BASE}/libraries/parse`, {
        method: 'POST',
      ...
      data.results.forEach((r: any) => {
        setLogs(prev => [...prev, `[CLOUD] ${r.file_name} 解析完成，${r.keyword_count} 个关键字已入库`]);
      });
      data.errors?.forEach((err: any) => {
        setLogs(prev => [...prev, `[ERROR] ${err.file}: ${err.error}`]);
      });
    } catch (err: any) {
      setLogs(prev => [...prev, `[ERROR] 上传失败: ${err.message}`]);
    } finally {
      setLibraryLoading(false);
      e.target.value = '';
    }
  ...
  // ─── 删除云端库 ─────────────────────────────────────────
  const handleDeleteLibrary = async (lib: LibraryFile) => {
    if (!confirm(`删除库 "${lib.file_name}"？这会从侧边栏移除对应关键字。`)) return;
    try {
      await apiFetch(`/libraries/${lib.id}`, { method: 'DELETE' }, token);
      setUserLibraryFiles(prev => prev.filter(f => f.id !== lib.id));
      // 从 library state 中移除该库的关键字
      setLibrary(prev => prev.filter(k => k.category !== lib.file_name));
      setLogs(prev => [...prev, `[INFO] 已删除库: ${lib.file_name}`]);
    } catch (e: any) {
      setLogs(prev => [...prev, `[ERROR] 删除失败: ${e.message}`]);
    }
  };

  // ─── 导出库为 .robot ────────────────────────────────────
  const handleExportLibrary = (lib: LibraryFile) => {
    window.open(`${API_BASE}/libraries/${lib.id}/export?token=${token}`, '_blank');
  };
  ...
  const handleLoadCase = (content: any, caseId: number, caseName: string) => {
    ...
    setCurrentCaseId(caseId); setCurrentCaseName(caseName);
  ...
  const setSteps = (updater: any) => {
    if (activeTab === 'testcases') {
      setTestCases(prev => prev.map(tc => {
        if (tc.id !== activeTestCaseId) return tc;
        return { ...tc, steps: typeof updater === 'function' ? updater(tc.steps) : updater };
      }));
    } else if (activeUserKeywordId) {
      setUserKeywords(prev => prev.map(kw => {
        if (kw.id !== activeUserKeywordId) return kw;
        return { ...kw, steps: typeof updater === 'function' ? updater(kw.steps) : updater };
      }));
    }
  };
  ...
  const handleImportRobot = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPendingImportFile(file); setShowImportConfirm(true); e.target.value = '';
  };
  ...
  const confirmImport = () => {
    ...
    reader.onload = (event) => {
      try {
        ...
        let parsedVars: any[] = [], parsedTCs: any[] = [], curTC: any = null, stack: any[] = [];
        ...
        const parseStep = (lineStr: string) => {
          ...
          let outVars: string[] = [], mod = '', kw = '', args: string[] = [], ci = 0;
          while (ci < parts.length && /^[@$&]\{/.test(parts[ci])) {
          ...
          if (ci < parts.length) { kw = parts[ci]; ci++; }
          if (!kw) return null;
          ...
          if (lib?.args) { lib.args.forEach((a: string, i: number) => { named[a] = i < args.length ? args[i] : ''; }); if (args.length > lib.args.length) extra = args.slice(lib.args.length); }
          else extra = [...args];
          return { id: `step_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`, keyword: kw, isCustomCode: false, isContainer: isCont, isComment: false, args: named, extraArgs: extra, modifier: mod, outputVars: outVars, children: isCont ? [] : undefined };
        };
        ...
        for (const line of lines) {
          ...
          if (t.startsWith('***')) {
            const sn = t.replace(/\*/g, '').trim().toLowerCase();
            finalize();
            if (sn.includes('settings')) section = 'settings';
            else if (sn.includes('variables')) section = 'variables';
            else if (sn.includes('test cases')) section = 'testcases';
            else if (sn.includes('keywords')) section = 'keywords';
            continue;
          }
          if (section === 'settings') { if (t) parsedSettings.push(line); }
            ...
            if (!line.startsWith(' ') && !line.startsWith('\t')) {
              finalize(); curTC = { id: `tc_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`, name: t, teardown: '', tags: '', steps: [] }; stack = [];
            } else if (curTC) {
              if (t.startsWith('[Tags]')) curTC.tags = t.replace('[Tags]', '').trim();
                ...
                const s = { id: `step_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`, keyword: '#', isComment: true, args: { text: t.substring(1).trim() } };
                (stack.length > 0 ? stack[stack.length - 1].children : curTC.steps).push(s);
              } else if (t.toUpperCase() === 'END') { stack.pop(); }
              else {
                const s = parseStep(t);
                if (s) {
                  if (['ELSE', 'ELSE IF'].includes(s.keyword.toUpperCase())) stack.pop();
                  (stack.length > 0 ? stack[stack.length - 1].children : curTC.steps).push(s);
                  if (s.isContainer) stack.push(s);
                }
              }
            }
          }
  ...
  const findStep = (nodes: any[], id: string): any => {
    for (let n of nodes) { if (n.id === id) return n; if (n.children) { const f = findStep(n.children, id); if (f) return f; } } return null;
  };
  const updateNode = (nodes: any[], id: string, updates: any): any[] =>
    nodes.map(n => n.id === id ? { ...n, ...updates } : n.children ? { ...n, children: updateNode(n.children, id, updates) } : n);
  const removeNode = (nodes: any[], id: string) => {
    let removed: any = null;
    const filter = (list: any[]): any[] => list.filter(n => { if (n.id === id) { removed = n; return false; } if (n.children) n.children = filter(n.children); return true; });
    return { newNodes: filter(nodes), removedNode: removed };
  };
  const insertNode = (nodes: any[], targetId: string | null, pos: string, newNode: any): any[] => {
    if (!targetId && pos === 'append') return [...nodes, newNode];
    let res: any[] = [];
    for (let n of nodes) {
      if (n.id === targetId) {
        if (pos === 'before') { res.push(newNode); res.push(n); }
        else if (pos === 'after') { res.push(n); res.push(newNode); }
        else if (pos === 'inside') res.push({ ...n, children: [...(n.children || []), newNode] });
      } else res.push(n.children ? { ...n, children: insertNode(n.children, targetId, pos, newNode) } : n);
    }
    return res;
  };
  ...
  const insertNewStep = (kw: any, tid: string | null, pos: string) => {
    ...
    if (kw.args) kw.args.forEach((a: string) => { defaultArgs[a] = kw.name === 'sshCommond' && a === 'channel' ? '${channel}' : kw.name === 'FOR' && a === 'IN' ? 'IN RANGE' : ''; });
    const s = { id: `step_${Date.now()}`, keyword: kw.name, isCustomCode: kw.isCustomCode || false, isContainer: kw.isContainer || false, isComment: kw.isComment || false, args: defaultArgs, extraArgs: [], modifier: '', customCode: '', outputVars: [], children: kw.isContainer ? [] : undefined };
    setSteps((p: any[]) => insertNode(p, tid, pos, s));
    setSelectedStepId(s.id);
  };

  const addUserKeyword = () => {
    const id = `kw_${Date.now()}`;
    setUserKeywords(prev => [...prev, { id, name: '新关键字', desc: '关键字描述', args: [], returnVars: [], steps: [] }]);
    setActiveUserKeywordId(id); setActiveTab('keywords');
  };
  const removeUserKeyword = (id: string) => {
    setUserKeywords(prev => prev.filter(k => k.id !== id));
  ...
  const handleStepDragOver = (e: React.DragEvent, id: string, isCont: boolean) => {
    e.preventDefault(); e.stopPropagation(); if (draggedStepId === id) return;
    const r = (e.currentTarget as HTMLElement).getBoundingClientRect(); const y = e.clientY - r.top;
    setDropTarget({ id, position: y < r.height * 0.25 ? 'before' : (isCont && y < r.height * 0.75) ? 'inside' : 'after' });
  };
  const handleStepDrop = (e: React.DragEvent, tid: string) => {
    e.preventDefault(); e.stopPropagation(); const dt = dropTarget; setDropTarget(null); setDraggedStepId(null);
    const sid = e.dataTransfer.getData('stepId'); const kd = e.dataTransfer.getData('application/json');
    if (sid) { if (sid !== tid) moveStep(sid, tid, dt?.position || 'after'); }
    else if (kd) insertNewStep(JSON.parse(kd), tid, dt?.position || 'after');
  };
  const handleCanvasDrop = (e: React.DragEvent) => {
    e.preventDefault(); e.stopPropagation(); setDropTarget(null); setDraggedStepId(null);
    const sid = e.dataTransfer.getData('stepId'); const kd = e.dataTransfer.getData('application/json');
    if (sid) moveStep(sid, null, 'append'); else if (kd) insertNewStep(JSON.parse(kd), null, 'append');
  };
  const handleDragOver = (e: React.DragEvent) => { e.preventDefault(); e.stopPropagation(); };
  const handleDragEnd = () => { setDraggedStepId(null); setDropTarget(null); };

  const updateStep = (id: string, updates: any) => setSteps((p: any[]) => updateNode(p, id, updates));
  const handleDelete = (e: React.MouseEvent, id: string) => {
    e.stopPropagation(); setSteps((p: any[]) => removeNode(p, id).newNodes);
    if (selectedStepId === id) setSelectedStepId(null);
  };
  const selectedStep = findStep(steps, selectedStepId || '');

  // ─── Code Gen ────────────────────────────────────────────
  const generateStepCode = (nodes: any[], level = 1): string => {
    let code = ''; const ind = '    '.repeat(level);
    nodes.forEach((s, i) => {
      if (s.isComment) { code += `${ind}# ${s.args.text}\n`; }
      else if (s.isCustomCode) { s.customCode.split('\n').forEach((l: string) => { code += `${ind}${l}\n`; }); }
        ...
        code += line + '\n';
        code += s.children?.length > 0 ? generateStepCode(s.children, level + 1) : `${ind}    # TODO\n`;
        const ku = s.keyword.toUpperCase();
        if (['FOR', 'WHILE', 'IF', 'ELSE IF', 'ELSE'].includes(ku)) {
          const next = nodes[i + 1];
          if (!(['IF', 'ELSE IF', 'ELSE'].includes(ku) && next && ['ELSE IF', 'ELSE'].includes(next.keyword.toUpperCase()))) code += `${ind}END\n`;
        }
      } else {
        let line = ind;
        const vv = (s.outputVars || []).filter((v: string) => v.trim());
        if (vv.length > 0) line += vv.join('    ') + '    '; else if (s.outputVar) line += `${s.outputVar}    `;
  ...
  const generateCode = (): string => {
    ...
    testCases.forEach(tc => {
      code += `${tc.name || 'Test Case'}\n`;
      if (tc.tags) code += `    [Tags]    ${tc.tags}\n`;
      code += generateStepCode(tc.steps);
    ...
    if (userKeywords.length > 0) {
      ...
      userKeywords.forEach(kw => {
        ...
        code += generateStepCode(kw.steps);
    ...
    if (customKeywordsSection.trim()) {
      if (!userKeywords.length) code += '*** Keywords ***\n';
  ...
  const handleDownload = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'test_suite.robot';
    document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  };

  const runSimulation = () => {
    setIsRunning(true); setShowCode(false);
    setLogs(['[INFO] Starting...', '[INFO] Parsing visual blocks...']);
    setTimeout(() => { setLogs(p => [...p, '[PASS] 执行完成']); setIsRunning(false); }, 1500);
  };

  // ─── Render Steps ────────────────────────────────────────
  const renderSteps = (nodes: any[], parentId: string | null = null): React.ReactNode => {
    if (!nodes?.length) {
      if (parentId) return <div className="py-4 text-center border-2 border-dashed border-gray-200 rounded bg-gray-50/50 text-gray-400 text-xs">拖拽模块到此内部</div>;
      return null;
    }
    return nodes.map(step => {
      ...
      if (step.isComment) return (
        <React.Fragment key={step.id}>
          {isDT && dropTarget!.position === 'before' && <div className="h-1 bg-[#F27D26] rounded my-1" />}
          <div draggable onDragStart={e => handleStepDragStart(e, step.id)} onDragOver={e => handleStepDragOver(e, step.id, false)} onDrop={e => handleStepDrop(e, step.id)} onDragEnd={handleDragEnd} onClick={e => { e.stopPropagation(); setSelectedStepId(step.id); }}
            className={`group relative flex items-center bg-green-50/50 border rounded-lg shadow-sm cursor-pointer mb-2 px-3 py-2 ${selectedStepId === step.id ? 'border-green-500 ring-1 ring-green-500' : 'border-green-200 hover:border-green-300'} ${draggedStepId === step.id ? 'opacity-50' : ''}`}>
          ...
          {isDT && dropTarget!.position === 'after' && <div className="h-1 bg-[#F27D26] rounded my-1" />}
        </React.Fragment>
      );
      return (
        <React.Fragment key={step.id}>
          {isDT && dropTarget!.position === 'before' && <div className="h-1 bg-[#F27D26] rounded my-1" />}
          <div draggable onDragStart={e => handleStepDragStart(e, step.id)} onDragOver={e => handleStepDragOver(e, step.id, step.isContainer)} onDrop={e => handleStepDrop(e, step.id)} onDragEnd={handleDragEnd} onClick={e => { e.stopPropagation(); setSelectedStepId(step.id); }}
            className={`group relative flex flex-col bg-white border rounded-lg shadow-sm cursor-pointer mb-2 ${selectedStepId === step.id ? 'border-[#F27D26] ring-1 ring-[#F27D26]' : 'border-gray-300 hover:border-gray-400'} ${draggedStepId === step.id ? 'opacity-50' : ''} ${isDT && dropTarget!.position === 'inside' ? 'ring-2 ring-[#F27D26]' : ''}`}>
            <div className="flex items-stretch">
              ...
              <button onClick={e => handleDelete(e, step.id)} className="w-10 flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-tr-lg opacity-0 group-hover:opacity-100"><Trash2 size={16} /></button>
            </div>
            {step.isContainer && (
  ...
  const combinedLibrary = [
    ...library,
    ...userKeywords.map(kw => ({ category: '用户自定义 (User Keywords)', name: kw.name, args: kw.args, desc: kw.desc, isContainer: false, isUserKeyword: true }))
  ];
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 bg-white border-r border-gray-300 flex flex-col shadow-sm z-10">
          <div className="p-3 border-b border-gray-200 space-y-2">
            ...
            {/* 上传 .robot/.resource 解析 */}
            <button
              onClick={() => libFileInputRef.current?.click()}
              disabled={libraryLoading}
              className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-[#F27D26]/10 hover:bg-[#F27D26]/20 border border-[#F27D26]/30 rounded text-[#F27D26] text-xs font-medium transition-colors disabled:opacity-50"
            >
              ...
              <div className="bg-gray-50 border border-gray-200 rounded p-2 space-y-1">
                ...
                {userLibraryFiles.map(lib => (
                  <div key={lib.id} className="flex items-center gap-1 group">
                    ...
                    <button onClick={() => handleExportLibrary(lib)} title="导出 .robot" className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-400 hover:text-blue-500">
                      <Download size={11} />
                    </button>
                    <button onClick={() => handleDeleteLibrary(lib)} title="删除" className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-400 hover:text-red-500">
                      <X size={11} />
                    </button>
                  </div>
                ))}
              ...
              <div key={cat}>
                ...
                <div className="space-y-1">
                  {filteredLibrary.filter(kw => kw.category === cat).map(kw => (
                    <div key={kw.name} draggable onDragStart={e => handleDragStart(e, kw)} onDoubleClick={() => insertNewStep(kw, null, 'append')}
              ...
              <div className="max-w-3xl mx-auto min-h-full pb-32">
                ...
                <div className="flex border-b border-gray-300 mb-6">
                  {(['testcases', 'keywords'] as const).map(tab => (
                    <button key={tab} onClick={() => { setActiveTab(tab); if (tab === 'keywords' && userKeywords.length > 0 && !activeUserKeywordId) setActiveUserKeywordId(userKeywords[0].id); }}
                      className={`px-6 py-3 text-sm font-bold border-b-2 transition-colors ${activeTab === tab ? 'border-[#F27D26] text-[#F27D26]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                ...
                {activeTab === 'testcases' ? (
                  <>
                    ...
                    <div className="flex items-end gap-1 mb-4 border-b border-gray-300 overflow-x-auto pt-2 px-2">
                      {testCases.map(tc => (
                        <div key={tc.id} onClick={() => setActiveTestCaseId(tc.id)}
                          ...
                          {testCases.length > 1 && (
                            <button onClick={e => { e.stopPropagation(); const n = testCases.filter(t => t.id !== tc.id); setTestCases(n); if (activeTestCaseId === tc.id) setActiveTestCaseId(n[0].id); }}
                              className="ml-1 text-gray-400 opacity-0 group-hover:opacity-100 hover:text-red-500"><X size={14} /></button>
                          )}
                        </div>
                      ))}
                      <button onClick={() => { const id = `tc_${Date.now()}`; setTestCases([...testCases, { id, name: `新用例 ${testCases.length + 1}`, teardown: '', tags: '', steps: [] }]); setActiveTestCaseId(id); }}
                        className="flex items-center gap-1 px-3 py-2 text-gray-500 hover:text-[#F27D26] hover:bg-gray-100 cursor-pointer whitespace-nowrap mb-[1px]">
                        <Plus size={14} /> 添加用例
                      </button>
                    </div>
                      ...
                      {showSettings && (
                        <div className="p-4 space-y-4">
                          <div className="pb-4 border-b border-gray-100 space-y-4">
                            <div>
                              <label className="block text-xs font-bold text-gray-700 mb-1">*** Settings ***</label>
                              <textarea value={settingsSection} onChange={e => setSettingsSection(e.target.value)} className="w-full px-2 py-1.5 text-xs font-mono border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]" rows={3} />
                            </div>
                            <div>
                              <label className="block text-xs font-bold text-gray-700 mb-1">全局变量</label>
                              <div className="space-y-2">
                                {globalVars.map((v, i) => (
                            ...
                            <div>
                              <label className="block text-xs font-bold text-gray-700 mb-1">*** Keywords *** (自定义)</label>
                              <textarea value={customKeywordsSection} onChange={e => setCustomKeywordsSection(e.target.value)} className="w-full px-2 py-1.5 text-xs font-mono border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]" rows={3} />
                            </div>
                          </div>
                          <div>
                  ...
                  <>
                    <div className="flex items-end gap-1 mb-4 border-b border-gray-300 overflow-x-auto pt-2 px-2">
                      {userKeywords.map(kw => (
                        <div key={kw.id} onClick={() => setActiveUserKeywordId(kw.id)}
                          className={`group flex items-center gap-2 px-4 py-2 rounded-t-lg border-t border-l border-r cursor-pointer whitespace-nowrap ${activeUserKeywordId === kw.id ? 'bg-white border-gray-300 text-[#F27D26] font-bold relative top-[1px]' : 'bg-gray-100 border-transparent text-gray-600 hover:bg-gray-200'}`}>
                    ...
                    {activeUserKeyword ? (
                      <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-6 overflow-hidden">
                        <div className="px-4 py-3 bg-gray-50 border-b border-gray-200"><h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider">关键字定义</h3></div>
                        <div className="p-4 space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              {activeUserKeyword.args.map((a: string, i: number) => (
                                <div key={i} className="flex gap-2 mb-2">
                                  <input type="text" value={a} onChange={e => { const n = [...activeUserKeyword.args]; n[i] = e.target.value; updateActiveUserKeyword({ args: n }); }} className="flex-1 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded text-xs" placeholder="${arg}" />
                                  <button onClick={() => updateActiveUserKeyword({ args: activeUserKeyword.args.filter((_: any, j: number) => j !== i) })} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 size={14} /></button>
                                </div>
                              ))}
                            ...
                            <div>
                              ...
                              {activeUserKeyword.returnVars.map((r: string, i: number) => (
                                <div key={i} className="flex gap-2 mb-2">
                                  <input type="text" value={r} onChange={e => { const n = [...activeUserKeyword.returnVars]; n[i] = e.target.value; updateActiveUserKeyword({ returnVars: n }); }} className="flex-1 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded text-xs" placeholder="${result}" />
                                  <button onClick={() => updateActiveUserKeyword({ returnVars: activeUserKeyword.returnVars.filter((_: any, j: number) => j !== i) })} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 size={14} /></button>
                                </div>
                              ))}
              ...
              <div className="space-y-5">
                ...
                {selectedStep.isComment ? (
                  <div className="pt-2 border-t border-gray-100">
                    <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3">注释内容</label>
                    <textarea value={selectedStep.args.text || ''} onChange={e => updateStep(selectedStep.id, { args: { text: e.target.value } })} className="w-full h-24 p-2 text-xs border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]" placeholder="输入注释..." />
                  </div>
                        ...
                        <div className="space-y-3">
                          {Object.keys(selectedStep.args).map(argName => (
                            <div key={argName}>
                              ...
                              <input type="text" value={selectedStep.args[argName]} onChange={e => updateStep(selectedStep.id, { args: { ...selectedStep.args, [argName]: e.target.value } })} className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:border-[#F27D26]" />
                            </div>
                          ))}
                        </div>
                      )}
